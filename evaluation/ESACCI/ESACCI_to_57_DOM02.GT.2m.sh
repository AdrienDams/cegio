...
for year in $( seq $startyear_esa $endyear_esa ) ; do # years available from ESACCI
 ...
 ncra -O -F -d levgrnd,11 -v $variable $modelfiles $scratch_ESA/$modelfile.top
 ncra -O -F -d levgrnd,12 -v $variable $modelfiles $scratch_ESA/$modelfile.bottom

 # Linear interpolation (value calculated by hand)
 ncflint -O -w 0.21,0.79 $scratch_ESA/$modelfile.top $scratch_ESA/$modelfile.bottom $scratch_ESA/$modelfile
